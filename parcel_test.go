...
func TestDeleteUncorrectStatus(t *testing.T) {
	...
	err = store.Delete(number)
	require.EqualError(t, err, "no rows has been deleted")
	p, err := store.Get(number)
	require.NoError(t, err)
...
func TestGetCountByClientID(t *testing.T) {
	// prepare
	db, err := sql.Open("sqlite", "tracker.db")
	if err != nil {
		fmt.Println(err)
	...
	// get by client
	count, err := store.getCountByClient(context.Background(), client) // получите список посылок по идентификатору клиента, сохранённого в переменной client
	// убедитесь в отсутствии ошибки
	// убедитесь, что количество полученных посылок совпадает с количеством добавленных
	require.NoError(t, err)
